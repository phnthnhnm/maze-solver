...
def create_settings_menu(
    ...
    if not isinstance(speed, list):
        raise TypeError(
            "Expected 'speed' to be a list, but got {}".format(type(speed).__name__)
        )
    ...
    algorithm_mapping = {"DFS": "dfs", "BFS": "bfs", "A*": "a_star"}
    ...
    dropdown = Dropdown(
        window._Window__root,
        list(algorithm_mapping.keys()),
    ...
    back_button = Button(
        window, "Back", screen_x // 2 - 50, screen_y // 2 + 100, 100, 50, on_back_click
    )
    back_button.hide()
    speed_input.hide()
    ...
    def show_settings_menu():
        ...
        seed_input.place(
            screen_x // 2 - 100, screen_y // 2, screen_x // 2 - 50, screen_y // 2
        )
        speed_input.place(
            screen_x // 2 - 100,
            screen_y // 2 + 50,
